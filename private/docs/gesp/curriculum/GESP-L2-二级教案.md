...
| 层次 | 名称 | 特点 | 典型容量/速度 |
|:---:|:---|:---|:---|
| 1 | 寄存器 | CPU内部，纳秒级访问 | 速度快，容量小，价格高 |
| 2 | CACHE | 高速缓存 | MB级 |
| 3 | RAM | 内存，掉电丢失 | GB级 |
| 4 | 硬盘/SSD | 外存，永久保存 | TB级 |
| 5 | 外设存储 | U盘、光盘、云存储 | 速度慢，容量大，价格低 |
...
| 层级 | 名称 | 主要协议 | 功能 |
|:---:|:---|:---|:---|
| 4 | 应用层 | HTTP、FTP、SMTP | 应用程序接口 |
| 3 | 传输层 | TCP、UDP | 端到端连接 |
| 2 | 网络层 | IP、ICMP、ARP | 寻址和路由 |
| 1 | 网络接口层 | 以太网、WiFi | 物理传输 |

**记忆口诀**：应传网接（从上层到下层）

**OSI七层模型（扩展了解）**：应用层→表示层→会话层→传输层→网络层→数据链路层→物理层
...
3. **初始化** → sum = 0, i = 1
4. **判断** i <= n? 
   - **是** → 执行累加 sum += i，然后 i++，返回步骤4继续判断
   - **否** → 输出 sum，程序结束

**循环过程**：i=1→2→3→...→n，每次将i加到sum中，直到i>n时停止
